  ...
  class PaymentResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BotFramework::PaymentResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BotFramework::PaymentResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'shipping_option')
        self.shipping_option = attributes[:'shipping_option']
      end

      if attributes.key?(:'payer_email')
        self.payer_email = attributes[:'payer_email']
      end

      if attributes.key?(:'payer_phone')
        self.payer_phone = attributes[:'payer_phone']
      end
    end
    ...
    def hash
      [method_name, details, shipping_address, shipping_option, payer_email, payer_phone].hash
    end
